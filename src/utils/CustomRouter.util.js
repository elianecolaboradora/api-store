...
import { applyPolicies } from "../middleware/applyPolicies.mid.js";
export class CustomRouter {
    ...
    applyMiddlewares = (middlewares) => {
        ...
        return middlewares.map((each) => {
            if (typeof each !== "function") {
                return (req, res, next) => next(); // Devuelve un middleware que simplemente pasa al siguiente
            }
    ...
    create = (path, policies, ...middlewares) =>
        this.router.post(path, applyPolicies(policies), this.applyMiddlewares(middlewares))
    read = (path, policies, ...middlewares) =>
        this.router.get(path, applyPolicies(policies), this.applyMiddlewares(middlewares))
    update = (path, policies, ...middlewares) =>
        this.router.put(path, applyPolicies(policies), this.applyMiddlewares(middlewares))
    destroy = (path, policies, ...middlewares) =>
        this.router.delete(path, applyPolicies(policies), this.applyMiddlewares(middlewares));
